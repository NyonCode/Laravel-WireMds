...
@php
    $breadcrumbs = app(\App\Support\Discovery\Services\BreadcrumbService::class)->generate($parameters);
@endphp
        ...
        @foreach($breadcrumbs as $index => $crumb)
            <li class="flex items-center {{ $itemClass }}">
                ...
                @if($crumb['active'] || !$crumb['url'])
                    <span class="{{ $crumb['active'] ? $activeClass : '' }}" @if($crumb['active']) aria-current="page" @endif>
                        {{ $crumb['label'] }}
                    </span>
                @else
